...
export default class CompanyManage extends React.Component {
  ...
  render() {
    return (
      <div>
        ...
        <SearchBar
          placeholder="请输入企业名称、联系人名称"
          maxLength={100}
          onSubmit={value => this.handleSearchCompany(value)}
        />
        <WhiteSpace />
        <StickyContainer>
          <Tabs
  ...
  _renderRowItem = (rowData, sectionID, rowID) => {
    ...
    return (
      ...
      >
        {company.name}
        <List.Item.Brief>{`${company.contact || ""} ${company.contactNo ||
          ""}`}</List.Item.Brief>
        <List.Item.Brief>{`${(company.province && `${company.province}省`) ||
          ""}
        ${(company.city && `${company.city}市`) || ""}${(company.district &&
          `${company.district}区`) ||
          ""}
        ${(company.street && `${company.street}街道`) ||
          ""}${company.detailAddress || ""}`}</List.Item.Brief>
      </List.Item>
    );
  };
  ...
  _getNewList = index => () => {
    let type = this.munuList[index].companyType;
    console.log(
      "_getNewList begin isRequesting is",
      this.isRequesting,
      "the type is ",
      type
    );
    if (!this.isRequesting) {
      ...
      getAllCompanyList(
        { page: this.munuList[index].pageIndex, companyType: type },
        true
      ).then(() => {
  ...
  handleSearchCompany = value => {
    getAllCompanyList({ name: value }); // _renderRowItem 没有触发 切换tab后的list列表也是没有变化的 跟全部内容一样
  };
}
